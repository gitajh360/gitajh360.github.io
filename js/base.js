...
var pageTimer = {} ; //定义计算器全局变量
	...
	$allname = {"cqssc":"重庆时时彩","xglhc":"香港六合彩","tjssc":"天津时时彩","xjssc":"新疆时时彩","bjpk10":"北京PK10","xyft":"幸运飞艇","jx115":"江西11选5","sh115":"上海11选5","xj115":"新疆11选5","js115":"江苏11选5","hlj115":"黑龙江11选5","fj115":"福建11选5","gd115":"广东11选5"};
function get_ld(){
...
xglhc = {"0":["xglhc_pt1","资料一区"],"1":["xglhc_pt2","资料二区"],"2":["xglhc_pt3","资料三区"],"3":["xglhc_tm","资料四区"],"4":["xglhc_sm","资料五区"]};
...
$.ajax({type:"get",url:donamess+"f="+a+"&n=new",dataType:"text",cache:false,success:function(news){
	...
	document.getElementById("optime").innerHTML = "时间:"+news["opentime"];
...
$("#menu").click(function(event){
    ...
    var jl_con = "";
    $.ajax({type:"GET",url:donamess+"f="+cp_st+"&n="+cp_st,async:true,datatype:'json',cache:false,success:function(data){
	data = JSON.parse(data);                
		for(var m in data){
if(m>10){}else{
			qishu  = data[m]["qishu"];//.slice(-3)
...
t1 = setInterval("getjl(cp_st)",8000); 	
 	...
 	if(document.getElementById("his_con").innerHTML==document.getElementById("his_con").innerHTML.split("iframe")){
document.getElementById("his_con").innerHTML='<iframe width="100%"  id="lhc_jl" frameborder="no"  src="http://18.163.60.203/kj/jl_2019.html?'+Math.random()+'"></iframe>'; 	}
...
document.getElementById("lhc").onclick =function(event){if(event.target.id=="lhc_c"){document.getElementById("lhc").style="display:none";}}

function lhc_kj(){
	document.getElementById("ts").style="display:block";
	if(confirm("香港六合彩马上开奖，点击确定看开奖直播")){
		
document.getElementById("lhc").style="display:block";
document.getElementById("lhc").innerHTML='<iframe src="http://18.163.60.203/kj/kj_1.html" width="100%" height="168px" frameborder="no"> </iframe><div id="lhc_c">最快六合彩开奖直播！点击收起</div>';
window.setTimeout(function(){document.getElementById("lhc").style="display:none";document.getElementById("lhc").innerHTML="";},300000);	
	}
	window.setTimeout(function(){document.getElementById("ts").style="display:none";},300000);
}
var ts = false;
document.getElementById("ts").onclick = function(){
	if(ts==false){document.getElementById("lhc").style="display:block";
document.getElementById("lhc").innerHTML='<iframe src="http://18.163.60.203/kj/kj_1.html" width="100%" height="168px" frameborder="no"> </iframe><div id="lhc_c">最快六合彩开奖直播！点击收起</div>';
window.setTimeout(function(){document.getElementById("lhc").style="display:none";document.getElementById("lhc").innerHTML="";},300000);	
	ts = !ts;
	}else{
		document.getElementById("lhc").style="display:none";
	document.getElementById("lhc").innerHTML="";
		ts = !ts;
}
...
$.ajax({
	...
	success:function(data){
		data = JSON.parse(data);
		if(data[0]==false){
			window.clearTimeout(t_kj);
		t_kj = 	window.setTimeout(lhc_kj,data[1]);
		}else{lhc_kj();}
	}
});
}
